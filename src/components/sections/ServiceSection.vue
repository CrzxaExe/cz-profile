<template>
    <section
        ...
        id="services" @mouseover="() => setSectionIndex(1)">
        <div
            ...
            <div class="lg:w-[60%]">
                ...
                <div class="mt-12 lg:mt-8 grid grid-cols-2 lg:grid-cols-4">
                    <div v-for="({ name, value }, i) in stats" :key="i" class="flex flex-col service-slide py-3 lg:py-0">
                        <span class="text-cyan-600 dark:text-cyan-500 text-4xl lg:text-5xl font-bold"><CountingUp :to="value" />+</span>
            ...
            <div class="w-full lg:w-[40%] flex flex-row justify-center items-center mt-8 lg:mt-0">
                <div class="w-full flex flex-col gap-3 lg:gap-1">
                    <div v-for="({ name, icon, description, total }, i) in interest"
                        class="min-h-full flex flex-row items-center px-4 lg:px-6 py-4 lg:py-5 gap-4 lg:gap-2 not-last:border-b-[1px]" :key="i">
                        <i :class="icon" class="text-2xl lg:text-3xl service-slide"></i>
                        <div class="flex flex-col gap-1 service-slide">
                            <h2 class="text-base lg:text-lg font-semibold relative overflow-hidden w-fit min-h-full">
                                <div class="bg-zinc-400 dark:bg-dark-900 w-full min-h-full absolute service-block"></div>
                                {{ name }}
                            </h2>
                            <div class="text-sm lg:text-xs inline-block font-light relative overflow-hidden w-fit min-h-full">
                                <div class="bg-zinc-400 dark:bg-dark-900 w-full min-h-full absolute service-block"></div>
                                {{ description }}
                            </div>

                        </div>
                        <CountingUp class="ml-auto text-cyan-600 dark:text-cyan-500 text-lg lg:text-xl" :to="total" />
                    </div>
                </div>
            </div>
        </div>
...
import CountingUp from '../items/CountingUp.vue';

function animation() {
    const tl = gsap.timeline({
        scrollTrigger: {
            trigger: ".service-slide",
            start: "top bottom",
            end: "+=10"
        },
    })

    tl.from(".service-slide", {
        y: 30,
        ease: "expo.out",
        opacity: 0,
        duration: 0.4,
